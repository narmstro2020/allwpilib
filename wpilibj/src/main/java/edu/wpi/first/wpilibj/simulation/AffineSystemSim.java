...
public class AffineSystemSim<States extends Num, Inputs extends Num, Outputs extends Num> {
  /** The affine system. */
  protected final AffineSystem<States, Inputs, Outputs> m_affineSystem;
  ...
  /** Constant vector. */
  protected final Matrix<States, N1> m_c;
  ...
  /**
   * Creates a simulated generic affine system with measurement noise from linear system components.
   *
   * @param affineSystem The linear system component.
  ...
  public AffineSystemSim(
      AffineSystem<States, Inputs, Outputs> affineSystem, double... measurementStdDevs) {
    if (measurementStdDevs.length != 0
        && measurementStdDevs.length
            != affineSystem.getLinearSystemComponent().getC().getNumRows()) {
    ...
    this.m_affineSystem = affineSystem;
    if (measurementStdDevs.length == 0) {
      m_measurementStdDevs =
          new Matrix<>(
              new SimpleMatrix(affineSystem.getLinearSystemComponent().getC().getNumRows(), 1));
    } else {
      m_measurementStdDevs = new Matrix<>(new SimpleMatrix(measurementStdDevs));
    }
    m_x =
        new Matrix<>(
            new SimpleMatrix(affineSystem.getLinearSystemComponent().getA().getNumRows(), 1));
    m_u =
        new Matrix<>(
            new SimpleMatrix(affineSystem.getLinearSystemComponent().getB().getNumCols(), 1));
    m_y =
        new Matrix<>(
            new SimpleMatrix(affineSystem.getLinearSystemComponent().getC().getNumRows(), 1));
    m_c =
  ...
  public void update(double dtSeconds) {
    // Update X. By default, this is the linear system dynamics X = Ax + Bu + c
   ...
   */
  public void setc(Matrix<States, N1> c) {
    for (int row = 0; row < c.getNumRows(); row++) {
      double value = c.get(row, 0);
      setc(row, value);
    }
  }
   ...
   * @param value The value to set the row to.
   */
  public void setc(int row, double value) {
    m_c.set(row, 0, value);
  }
  ...
  protected Matrix<States, N1> updateX(
    ...
    return m_affineSystem.calculateX(currentXhat, u, m_c, dtSeconds);
  }
